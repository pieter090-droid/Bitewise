-- Fase 1 audit, batch 2 (hartig) — deel 4: leesronde cheese_snacks (509).
-- Elk product individueel gelezen. cheese_snacks is de facto de kaasfamilie
-- (kaasblokken, plakken, geraspt, verse kazen, borrel-kaasbiscuits en
-- kaasvervangers blijven hier bewust staan).
--
-- Hoofdbevindingen (wortel: kaasregel matcht alles met kaas/cheese in de
-- naam — R17 voegt exclusies toe, R17b zet grillworst in de vleeswarenregel):
--  K1 16 vleeswaren (15 grillworst-varianten + carpaccioschotel) ->
--     cold_cuts.
--  K2 12 maaltijden (mac & cheese x5, pizza x2, maaltijdsalades, gratin,
--     Ella's babymaaltijd, Philly pasta) -> ready_meals; chicken fingers +
--     verse girasoli -> meal_components; Starbucks muffin -> sandwiches.
--  K3 7 gefrituurde kaassnacks (Mora bytes/bites/souffles, Kwekkeboom
--     croquetten, mozzarella sticks x2, McCain pickers) -> fried_snacks.
--  K4 5 kaas-zoutjes/chips (flips, crispers, corn snacks, snackmix,
--     nacho's) -> crisps_chips; 3 oven sticks/zoutjes/Sultana cracker ->
--     crackers_rice_cakes.
--  K5 3 dips -> sauces_dips; 4 spread-producten (zuivelspread x2, pesto-
--     kaasspread, oude-kaassalade) -> savory_spreads; roomkazen blijven
--     bewust in cheese_snacks (Philadelphia/St Moret-groep staat daar).
--  K6 3 vegetarische schnitzels/filets (Vivera, rondo's) ->
--     meat_alternatives_non_swap (niet swapbaar).
--  K7 4 review_required (Peppers&Cheese, rozijnenbol kaas x2, Vegan Deli).
--
-- `products` wordt nergens aangeraakt. `products` blijft raw.

create table if not exists public._snapshot_0084_before as
...
where barcode in (
  '8719587217160','8718907978347','8718907778978','8718907391658','8719587035115',
  '8719587010952','2378338402758','2311997402523','2313139902748','2352086802794',
  '2302538305076','2352086102771','2302538304260','20975760','8714798300793',
  '8718452682935',
  '8717953112415','8718907444187','8718989028749',
  '4065019126246','5900571104952','8711299015386','8718452904396','8718452897568',
  '8710532474317','8718906979147','8710412045507',
  '8710861912740','8710861913051','8719009380410','8710861995194','8414204722057',
  '4056489387893','8710438126440',
  '8719587225011','8712076089729','5060503505445','8719587002292','8718906186057',
  '8718907473736','8719587206935','8721317780349','8281594791822','0613668024709',
  '6150830097047','8720326086589',
  '8720195573555','8710624317959','8719128167220',
  '8718989007942','8718265507906','8718452923281','8719587216798',
  '8718300871450','4068428040431','4056489219507',
  '8713576001259','8714368018677','8710400267980','5420005733256'
);

update public.product_features set swap_family='cold_cuts', classification_confidence=0.6, classified_at=now(),
  classification_reason='audit1_0084: grillworst/carpaccio is vleeswaren, geen kaas (wortel gefixt in R17/R17b)'
where barcode in (
  '8719587217160','8718907978347','8718907778978','8718907391658','8719587035115',
  '8719587010952','2378338402758','2311997402523','2313139902748','2352086802794',
  '2302538305076','2352086102771','2302538304260','20975760','8714798300793',
...
update public.product_features set swap_family='sauces_dips', classification_confidence=0.55, classified_at=now(),
  classification_reason='audit1_0084: kaasdip is een dip, geen kaas'
where barcode in ('8717953112415','8718907444187','8718989028749');

update public.product_features set swap_family='crisps_chips', classification_confidence=0.55, classified_at=now(),
  classification_reason='audit1_0084: kaas-flips/crispers/nachochips/snackmix zijn chips-zoutjes, geen kaas'
where barcode in (
  '4065019126246','5900571104952','8711299015386','8718452904396','8718452897568');

update public.product_features set swap_family='crackers_rice_cakes', classification_confidence=0.5, classified_at=now(),
  classification_reason='audit1_0084: oven sticks/ovenzoutjes/hartige Sultana cracker horen bij crackers (zoutjes-groep)'
where barcode in ('8710532474317','8718906979147','8710412045507');

update public.product_features set swap_family='fried_snacks', classification_confidence=0.55, classified_at=now(),
  classification_reason='audit1_0084: gefrituurde kaassnack (bites, souffles, croquetten, mozzarella sticks, pickers) hoort bij fried_snacks'
where barcode in (
  '8710861912740','8710861913051','8719009380410','8710861995194','8414204722057',
  '4056489387893','8710438126440');

update public.product_features set swap_family='ready_meals', classification_confidence=0.55, classified_at=now(),
  classification_reason='audit1_0084: mac & cheese/pizza/maaltijdsalade/gratin met kaas is een maaltijd, geen kaas'
where barcode in (
  '8719587225011','8712076089729','5060503505445','8719587002292','8718906186057',
  '8718907473736','8719587206935','8721317780349','8281594791822','0613668024709',
  '6150830097047','8720326086589');
...
update public.product_features set swap_family='meal_components', classification_confidence=0.55, classified_at=now(),
  classification_reason='audit1_0084: chicken fingers/verse gevulde pasta zijn maaltijdcomponenten'
where barcode in ('8720195573555','8710624317959');

update public.product_features set swap_family='sandwiches_wraps', classification_confidence=0.5, classified_at=now(),
  classification_reason='audit1_0084: egg & cheese muffin is een belegd broodje'
where barcode in ('8719128167220');

update public.product_features set swap_family='savory_spreads', classification_confidence=0.55, classified_at=now(),
  classification_reason='audit1_0084: zuivelspread/kaasspread/smeersalade is hartig broodbeleg; roomkazen blijven bewust in cheese_snacks'
where barcode in ('8718989007942','8718265507906','8718452923281','8719587216798');
...
update public.product_features set swap_family='meat_alternatives_non_swap', is_swap_relevant=false, classification_confidence=0.55, classified_at=now(),
  classification_reason='audit1_0084: vegetarische schnitzel/filet/rondo is een vleesvervanger, geen snack'
where barcode in ('8718300871450','4068428040431','4056489219507');
...
update public.product_features set swap_family=null, classification_status='review_required',
  ...
  classification_reason='audit1_0084: producttype onduidelijk (Peppers&Cheese; rozijnenbol kaas-combinatie x2; Vegan Deli smoked: kaas of vleesvervanger)'
where barcode in ('8713576001259','8714368018677','8710400267980','5420005733256');

-- R17/R17b: regelwortel-fixes, volledige functie hieronder.
-- POSTFLIGHT: select count(*) from product_features where classification_reason like 'audit1_0084%'; -- 60
-- ROLLBACK: herstel via _snapshot_0084_before.
